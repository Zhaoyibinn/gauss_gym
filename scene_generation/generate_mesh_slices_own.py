...
def preprocess_transforms_colmap(
  load_dir: Path,
):
  sparse_dir = load_dir / 'sparse' / '0'
  cameras_txt = sparse_dir / 'cameras.txt'
  images_txt = sparse_dir / 'images.txt'
  images_dir = load_dir / 'images'
  ...
  if not images_dir.exists():
    raise FileNotFoundError(
      f'Missing image directory referenced by COLMAP export: {images_dir}'
    )

  def _parse_intrinsics() -> dict:
    intrinsics = {}
    ...
    if not intrinsics:
      raise ValueError(f'No valid camera entries found in {cameras_txt}')
    return intrinsics
  ...
  for idx in range(0, len(lines), 2):
    ...
    if len(entries) < 10:
      raise ValueError(f'Invalid image entry in COLMAP images.txt: {lines[idx]}')

    image_id = int(entries[0])
    qw, qx, qy, qz = map(float, entries[1:5])
    tx, ty, tz = map(float, entries[5:8])
    camera_id = int(entries[8])
    image_name = ' '.join(entries[9:])

    if camera_id not in camera_intrinsics:
      raise KeyError(
        f'Camera id {camera_id} referenced by image {image_id} not found in cameras.txt'
      )
    ...
    # COLMAP stores world-to-camera rotation (R_cw) and translation (t_cw).
    quat_xyzw = np.array([qx, qy, qz, qw], dtype=np.float64)
    R_cw = vtf.SO3.from_quaternion_xyzw(quat_xyzw).as_matrix()
    ...
    transform_matrix[:3, :3] = R_wc
    transform_matrix[:3, 3] = camera_center

    depth_path = load_dir / 'depth' / Path(image_name).with_suffix('.png')
  ...
  transforms.sort(key=lambda frame: frame['file_path'])
  return transforms
